    ...
    , isDrawROI(true)
{
    ...
    m_pDelAction = new QAction("del", this);
...
void GraphicsView::clear_all()
{
    m_drawROI->DeleteAllROIS();
}
std::map<int, QRect> GraphicsView::getRoi()
{
    return m_drawROI->getRoiRect();
}
...
void GraphicsView::mousePressEvent(QMouseEvent* event)
{
    ...
    qDebug()<<viewWidth<<","<<viewHeight<<","<<fviewWidth<<","<<fviewHeight<<","<<pviewWidth<<","<<pviewHeight<<"\n";
    //鼠标左键按下
...
void GraphicsView::setBackImage(QImage img)
{
    ...
    float w_s = (float)(640.0/width);
    float h_s = (float)(640.0/height);
    float scale = w_s < h_s? w_s:h_s;
    m_scaledValue = scale;
    qDebug()<<"width="<<width<<",height="<<height<<",scale="<<scale<<"\n";
    QSize s = QSize(width * scale, height*scale);//this->size();
