...
class HttpClientPrivate {
    ...
    * @param method 请求的类型
    */
    static QMetaObject::Connection executeQuery(HttpClientPrivate *d, HttpClientRequestMethod method);
...
// 执行请求的辅助函数
QMetaObject::Connection HttpClientPrivate::executeQuery(HttpClientPrivate *d, HttpClientRequestMethod method) {
    ...
    // [4] 请求结束时获取响应数据，在 handleFinish 中执行回调函数
    // 请求结束时一次性读取所有响应数据
    auto ret = QObject::connect(reply, &QNetworkReply::finished, [=] {
...
// 执行 POST 请求
QMetaObject::Connection HttpClient::post() {
    return HttpClientPrivate::executeQuery(d, HttpClientRequestMethod::POST);
}
